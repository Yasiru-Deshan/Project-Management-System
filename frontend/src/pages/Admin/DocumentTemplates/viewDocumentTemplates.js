...
export default function ViewDocumentTemplates() {
  ...
  useEffect(() => {
    const getFilesList = async () => {
      try {
        const { data } = await axios.get('http://localhost:8070/admin/doctemplate/getAllFiles');
        setErrorMsg('');
        setFilesList(data);
      } catch (error) {
  ...
  const downloadFile = async (id, path, mimetype) => {
    try {
      const result = await axios.get(`http://localhost:8070/admin/doctemplate/download/${id}`, {
        responseType: 'blob'
      });
  ...
  const deleteFile = async(id) => {
    ...
    if(window.confirm("Are you sure about deleting this file?")){
        deletion = await axios.delete(`http://localhost:8070/admin/doctemplate/delete/${id}`);
    }
    if(deletion) {
        window.location = "/admin/doctemplate/view"
    }    
  }
  ...
  return (
    <div className="it19184722-myForm-adminDashboard">
      ...
      <div className="files-container">
        <Link to="/admin/doctemplate/upload" className="btn it19184722-green-btn-upload">
                ...
                ({ _id, title, description, file_path, file_mimetype }) => (
                  <tr key={_id} className="it19184722-tr">
                    ...
                    <td className="it19184722-td-btn">
                      ...
                      <button onClick={() => deleteFile(_id)} className="btn btn-danger it19184722-mybtn it19184722-red-btn ">
