...
export default function ViewSubmissions() {
  ...
  useEffect(() => {
    const getFilesList = async () => {
      try {
        const { data } = await axios.get('http://localhost:8070/admin/submissions/getAllFiles');
        setErrorMsg('');
        setFilesList(data);
      } catch (error) {
  ...
  const downloadFile = async (id, path, mimetype) => {
    try {
      const result = await axios.get(`http://localhost:8070/admin/submissions/download/${id}`, {
        responseType: 'blob'
      });
  ...
  const deleteFile = async(id) => {
    ...
    if(window.confirm("Are you sure about deleting this file?")){
        deletion = await axios.delete(`http://localhost:8070/admin/submissions/delete/${id}`);
    }
    if(deletion) {
        window.location = "/admin/submissions/view"
    }    
}
  ...
  return (
    <div className="it19184722-myForm-adminDashboard">
      ...
      <div className="files-container">
        <Link to="/admin/submissions/upload" className="btn it19184722-green-btn-upload">
